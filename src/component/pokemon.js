    ...
    return(
        ...
        <Card className={`${pokemon.types[0].type.name}`}>
            ...
            <CardInfo>
                ...
                {pokemon.name.length <11 ?<h2>{pokemon.name.toUpperCase().replace("-"," ")}</h2>:<h2>{pokemon.name.substring(0,10).toUpperCase().replace("-"," ")}...</h2> }
                <Span>{pokemon.types.map(e=><p key={e.type.name} >{e.type.name.toUpperCase()}</p>)}</Span>
            </CardInfo>
        </Card>
        </Link> 
    )
